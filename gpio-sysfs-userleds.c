...
ssize_t direction_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if(sysfs_streq(buf,"in"))
        gpiod_direction_input(dev_data->desc);
...
{
    &dev_attr_direction.attr,   /*comes from the expansion of above macro*/
    &dev_attr_value.attr,
    &dev_attr_label.attr,
    NULL
};
...
/* this is required to be provided to the device_create_with_groups function */
...
{ 
    ...
    /*Get the number of child nodes so that we can allocate memory in driver private struct*/
    ...
    {
        /*Create a private data structure for each child node */
        ...
        /* assign label from child node and store it in the device private data structure */
        ...
        /* get hold of the gpiod descriptor , so we can do operations in it later */
